...
/// <summary>
/// This class is responsible for the animations that happen in the Deposition 2 Chamber.
/// </summary>
public class Deposition2 : MonoBehaviour {
	...
	public void resetDeposition() {
		...
		GameObject[] materialAtoms = GameObject.FindGameObjectsWithTag ("MaterialAtom2");
	...
	void Update () {
		...
		// Check if glass is in deposition 2 and not paused
		if (AlertObserver.glassInDeposition2 && !Demonstration.paused) {
		...
		} else if (!AlertObserver.glassInDeposition2) {
			// Reset deposition if glass is not in deposition 2
			resetDeposition ();
		}
	...
	void detectCollisionWithMaterial() {
		...
		foreach (GameObject atom in argonAtoms) {
			...
			if (atom.transform.localPosition.x == 0.13f) {
	...
	void fadeAtoms() {
		...
		glassColorAnimator.SetTrigger ("silver");

		if (AlertObserver.glassColor.Equals ("silver")) {
			// Move the glass out of the machine once it has changed color
			glassAnimator.SetTrigger ("moveToFinal");
